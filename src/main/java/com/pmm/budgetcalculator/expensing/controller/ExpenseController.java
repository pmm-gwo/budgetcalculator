...
public class ExpenseController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> removeExpenseById(@PathVariable("id") Long id) {
        expenseService.removeExpenseById(id);
        return ResponseEntity.ok("Entity deleted");
    }

    @PutMapping("/{id}")
    public ResponseEntity<ExpenseDto> updateExpense(@PathVariable("id") Long id, @RequestBody ExpenseDto expenseDetailsDto) {
